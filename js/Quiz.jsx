...
function Dashboard({ user, userProfile, onStartNew, onContinueQuiz, showAlert, showConfirm, showPrompt }) {
        ...
        showPrompt("請輸入新資料夾名稱：", "", (name) => {
            ...
            if(cleanName && !userFolders.includes(cleanName)) {
                db.collection('users').doc(user.uid).set({
                    folders: firebase.firestore.FieldValue.arrayUnion(cleanName)
                }, { merge: true }).then(() => {
    ...
    const moveQuizToFolder = (quiz, targetFolder) => {
        db.collection('users').doc(user.uid).collection('quizzes').doc(quiz.id).update({ folder: targetFolder })
        .then(() => {
    ...
    const handleGenerateCode = async (quiz) => {
        ...
        try {
            await db.collection('shareCodes').doc(newCode).set({
                ownerId: user.uid,
                quizId: quiz.id
            });
            await db.collection('users').doc(user.uid).collection('quizzes').doc(quiz.id).update({
                shortCode: newCode
            });
    ...
    showPrompt("請輸入 6 碼測驗代碼：", "", async (code) => {
        ...
        try {
            // 1. 【本地重複檢查】檢查目前已經載入的 records 中是否已有相同代碼
            const isDuplicateCode = records.some(r => r.shortCode === cleanCode);
            if (isDuplicateCode) {
                return showAlert(`⚠️ 匯入失敗：\n您的題庫中已存在代碼為 [${cleanCode}] 的試卷，不可重複匯入。`, "重複加入");
            }
            ...
            // 3. 【身分檢查】檢查你是不是這份試卷的原始擁有者
            if (ownerId === user.uid) {
                return showAlert("⚠️ 這是您自己建立的試卷：\n該試卷已在您的題庫中，不需要透過代碼匯入。", "重複擁有");
            }

            // 4. 【遠端資料檢查】從原作者的 collection 中抓取資料
           ...
           // 5. 【內容唯一性檢查】使用更寬鬆但精確的比對
const isContentDuplicate = records.some(r => {
    // 把名稱裡的 "(來自匯入)" 或 "(來自代碼)" 拿掉再比對
    const cleanLocalName = r.testName.replace(/\s*\(來自.*\)/, '').trim();
    const cleanIncomingName = data.testName.trim();
    
    // 強制轉成數字比對題數
    const isSameName = cleanLocalName === cleanIncomingName;
    const isSameCount = Number(r.numQuestions) === Number(data.numQuestions);
    
    return isSameName && isSameCount;
});

if (isContentDuplicate) {
    // 這裡一定要 return 才會終止後面的 add 動作
    return showAlert(`⚠️ 發現內容重複：\n您的題庫中已有一份內容完全相同的試卷 [${data.testName}]。`, "內容重複");
}
                            ...
                            <div>
                                <div className="flex justify-between items-start mb-2">
                                    <h2 className="font-bold text-lg leading-tight pr-2 flex flex-wrap items-center gap-1 dark:text-white">
                                        ...
                                        {rec.isShared && <span className="text-xs bg-orange-100 dark:bg-orange-900 text-orange-800 dark:text-orange-200 px-1.5 py-0.5 no-round align-middle font-normal whitespace-nowrap">好友分享</span>}
...
function QuizApp({ currentUser, userProfile, activeQuizRecord, onBackToDashboard, showAlert, showConfirm, showPrompt }) {
    ...
    const [pdfZoom, setPdfZoom] = useState(1); // 新增功能: PDF 縮放
    ...
    const handleRetake = () => {
        showConfirm("確定要再做一次嗎？\n先前的分數將保留在您的歷史紀錄中，系統將為您清空目前答案，不產生新試卷檔案。", () => {
            ...
            db.collection('users').doc(currentUser.uid).collection('quizzes').doc(quizId).update({
    ...
    const handleSyncQuestionText = async () => {
        ...
        try {
            ...
            if (latestSharedTo.length > 0) {
                const promises = latestSharedTo.map(target => {
                    return db.collection('users').doc(target.uid).collection('quizzes').doc(target.quizId).update({ questionText: questionText });
                });
                await Promise.all(promises);
        ...
        try {
            ...
            if (isShared && !initialRecord.results) {
                ...
                await db.collection('users').doc(currentUser.uid).update({
                    mcData: { ...mcData, diamonds: mcData.diamonds + 50, exp: newExp, level: newLevel }
                });
                        ...
                        <div className="flex-grow w-full relative bg-gray-200 dark:bg-gray-800 flex flex-col overflow-auto">
                            ...
                            {questionText && (
                                <div className={`w-full relative bg-white dark:bg-gray-800 flex flex-col flex-grow h-full`}>
                                    <textarea 
                                        className={`absolute inset-0 w-full h-full p-4 resize-none outline-none custom-scrollbar text-sm leading-relaxed ${isShared ? 'bg-gray-50 dark:bg-gray-900 text-gray-700 dark:text-gray-300' : 'bg-white dark:bg-gray-800 text-black dark:text-white focus:ring-2 focus:ring-inset focus:ring-black dark:focus:ring-white'}`}
                                        style={{ whiteSpace: 'pre-wrap' }}
                                        value={questionText}
                                        onChange={e => setQuestionText(e.target.value)}
                                        readOnly={isShared}
                                        placeholder={isShared ? "沒有提供試題文字" : "在此輸入或貼上試題純文字..."}
    ...
    if (step === 'grading') return (
        <div className="flex flex-col min-h-[100dvh] items-center justify-center p-4 relative py-10 overflow-y-auto bg-gray-100 dark:bg-gray-900 transition-colors">
            ...
            <div className="bg-white dark:bg-gray-800 p-8 shadow-md w-full max-w-lg no-round border border-gray-200 dark:border-gray-700 mt-10 transition-colors">
                ...
                <textarea 
                    className={`w-full h-40 p-3 border border-gray-300 dark:border-gray-600 no-round font-mono mb-4 outline-none tracking-widest text-lg uppercase custom-scrollbar ${isShared ? 'bg-gray-100 dark:bg-gray-900 text-gray-500 dark:text-gray-400 cursor-not-allowed' : 'bg-white dark:bg-gray-700 text-black dark:text-white focus:border-black dark:focus:border-white'}`} 
                    placeholder="例如: ABCD..." 
                    ...
                    readOnly={isShared}
                ></textarea>
                {isShared && <p className="text-orange-500 dark:text-orange-400 text-xs font-bold mb-4 -mt-2">🔒 這是好友分享的解答，為確保成績準確已鎖定編輯。</p>}
    ...
    if (step === 'results') return (
        <div className="flex flex-col h-[100dvh] bg-gray-100 dark:bg-gray-900 p-2 sm:p-4 w-full overflow-hidden transition-colors">
                    ...
                    {!isShared && (
                        <button onClick={async () => {
                            if (shortCode) {
                                ...
                                const newCode = Math.random().toString(36).substring(2, 8).toUpperCase();
                                try {
                                    await db.collection('shareCodes').doc(newCode).set({ ownerId: currentUser.uid, quizId: quizId });
                                    await db.collection('users').doc(currentUser.uid).collection('quizzes').doc(quizId).update({ shortCode: newCode });
            ...
            >
                ...
                <div className={`flex-grow flex flex-col bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-sm no-round overflow-hidden transition-colors`}>
                    <div className="flex-grow overflow-y-auto p-4 sm:p-6 custom-scrollbar bg-gray-50 dark:bg-gray-900 transition-colors">
                        <div className="bg-white dark:bg-gray-800 p-6 shadow-sm border-t-4 border-black dark:border-gray-500 text-center mb-6 no-round transition-colors">
                            <h2 className={`text-5xl sm:text-6xl font-black mb-4 ${results.score >= 60 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>{results.score}</h2>
                            <p className="text-gray-500 dark:text-gray-400 font-bold tracking-widest">答對 {results.correctCount} / {results.total} 題</p>
                        </div>
                        
                        <div className="bg-white dark:bg-gray-800 p-4 sm:p-6 border border-gray-200 dark:border-gray-700 no-round shadow-sm transition-colors">
                            <div className="flex justify-between items-center mb-6 border-b border-gray-100 dark:border-gray-700 pb-3">
                                <h3 className="font-bold text-lg dark:text-white">答案解析</h3>
                                <div className="flex space-x-2">
                                    <button onClick={() => { setShowOnlyStarred(!showOnlyStarred); setShowOnlyWrong(false); }} className={`text-sm font-bold px-3 py-1 no-round border transition-colors ${showOnlyStarred ? 'bg-orange-100 dark:bg-orange-900 text-orange-700 dark:text-orange-300 border-orange-200 dark:border-orange-700' : 'bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400 border-gray-200 dark:border-gray-600 hover:text-black dark:hover:text-white'}`}>
                                        {showOnlyStarred ? '顯示全部' : '⭐ 只看星號'}
                                    </button>
                                    <button onClick={() => { setShowOnlyWrong(!showOnlyWrong); setShowOnlyStarred(false); }} className={`text-sm font-bold px-3 py-1 no-round border transition-colors ${showOnlyWrong ? 'bg-red-100 dark:bg-red-900 text-red-700 dark:text-red-300 border-red-200 dark:border-red-700' : 'bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400 border-gray-200 dark:border-gray-600 hover:text-black dark:hover:text-white'}`}>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '8px 12px' }}>
                                {results.data.filter(q => {
                                    ...
                                    <div key={q.number} className={`p-2 sm:p-3 border no-round flex justify-between items-center ${q.isCorrect ? 'bg-green-50 dark:bg-green-900/30 border-green-100 dark:border-green-800' : 'bg-red-50 dark:bg-red-900/30 border-red-200 dark:border-red-800'} ${q.isStarred ? 'ring-2 ring-orange-300 dark:ring-orange-500' : ''}`}>
                                        <span className={`font-black text-sm flex items-center ${q.isCorrect ? 'text-green-800 dark:text-green-400' : 'text-red-800 dark:text-red-400'}`}>
                                            {q.number}.
                                            {q.isStarred && <span className="text-orange-500 ml-1.5 text-xs">★</span>}
                                        </span>
                                        <div className="text-sm font-bold flex items-center">
                                            <span className={q.isCorrect ? 'text-gray-700 dark:text-gray-300' : 'text-red-600 dark:text-red-400 line-through'}>{q.userAns}</span>
                                            <span className="mx-2 text-gray-300 dark:text-gray-600">→</span>
                                            <span className="text-green-700 dark:text-green-400">{q.correctAns}</span>
                                        </div>
                                    </div>
                                ))}
                            </div>
