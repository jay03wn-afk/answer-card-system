...
window.QlibDashboard = function QlibDashboard({ user, userProfile, showAlert, showConfirm, showPrompt, onContinueQuiz }) {
    ...
    const [isPublishing, setIsPublishing] = useState(null); // 上架表單狀態
    ...
    const [isGlobalSearch, setIsGlobalSearch] = useState(false); // ✨ 全域搜尋狀態
    ...
    useEffect(() => {
        if (activeMainTab === 'public') {
            const unsub = window.db.collection('publicQlib').doc('main').onSnapshot(async docSnap => {
                if (!docSnap.exists) { setPublicSubjects([]); return; }
                let loadedSubjects = docSnap.data().subjects || [];
                try {
                    const qSnap = await window.db.collection('publicQlib_questions').get();
                    const qDict = {};
                    qSnap.forEach(d => { qDict[d.id] = d.data().questions || []; });

                    const statSnap = await window.db.collection('publicQlib_stats').get();
                    const statsDict = {};
                    ...
                    loadedSubjects = loadedSubjects.map(s => ({
                        ...
                        chapters: (s.chapters || []).map(c => ({
                            ...c,
                            questions: (qDict[c.id] || c.questions || []).map(q => ({
                                ...q,
                                stats: statsDict[q.id] || null
                            }))
                        }))
                    }));
                } catch(e) { console.error(e); }
                setPublicSubjects(loadedSubjects);
            });
            return () => unsub();
    ...
    const handleImport = () => {
        ...
        const newSubjects = (targetList || []).map(s => {
            if (s.id === activeSubjectId) {
                return {
                    ...s, chapters: (s.chapters || []).map(c => c.id === activeChapterId ? { ...c, questions: [...(c.questions || []), ...parsedQuestions] } : c)
                };
            }
            return s;
        });
    ...
    const handleClearQuestions = () => {
        ...
        showConfirm("確定要清空此章節嗎？無法還原。", () => {
            ...
            const newSubjects = (targetList || []).map(s => {
                if (s.id === activeSubjectId) {
                    return { ...s, chapters: (s.chapters || []).map(c => c.id === activeChapterId ? { ...c, questions: [] } : c) };
                }
                return s;
            });
            saveToDb(newSubjects, isPublic);
    ...
    const handleDeleteQuestion = (qId) => {
        ...
        showConfirm("確定要刪除這題嗎？", () => {
            ...
            const newSubjects = (targetList || []).map(s => {
                if (s.id === activeSubjectId) {
                    return { ...s, chapters: (s.chapters || []).map(c => c.id === activeChapterId ? { ...c, questions: (c.questions || []).filter(q => q.id !== qId) } : c) };
                }
                return s;
            });
            saveToDb(newSubjects, isPublic);
    ...
    const handleSaveEdit = () => {
        ...
        const newSubjects = (targetList || []).map(s => {
            if (s.id === activeSubjectId) {
                return { ...s, chapters: (s.chapters || []).map(c => c.id === activeChapterId ? { ...c, questions: (c.questions || []).map(q => q.id === editingQuestion.id ? editingQuestion : q) } : c) };
            }
            return s;
        });
        saveToDb(newSubjects, isPublic);
        ...
        showConfirm(`確定要把「${subj.name} - ${isPublishing.chap.name}」推送到公開題庫嗎？\n推送後所有人都可免費使用。`, async () => {
            try {
                ...
                if (targetSubjIdx !== -1) {
                    ...
                    currentPublicSubjects[targetSubjIdx].chapters.push({
                        id: isPublishing.chap.id,
                        name: isPublishing.chap.name
                    });
                } else {
                    targetSubjId = Date.now().toString();
                    currentPublicSubjects.push({
                        id: targetSubjId,
                        name: subj.name,
                        chapters: [{
                            id: isPublishing.chap.id,
    ...
    const currentSubjectList = activeMainTab === 'public' ? publicSubjects : subjects;
    const activeSubject = (currentSubjectList || []).find(s => s.id === activeSubjectId);
    const activeChapter = (activeSubject?.chapters || []).find(c => c.id === activeChapterId);

    let sourceQuestions = [];
    if (isGlobalSearch) {
        (currentSubjectList || []).forEach(s => {
            (s.chapters || []).forEach(c => {
                (c.questions || []).forEach((q, idx) => {
                    sourceQuestions.push({ ...q, _subjName: s.name, _chapName: c.name, _localNum: idx + 1 });
                });
            });
        });
    } else {
    ...
    const displayedQuestions = sourceQuestions.filter(q => {
        ...
        const matchSearch = !searchQ || q.text.toLowerCase().includes(lowerSearch) || (q.explain || '').toLowerCase().includes(lowerSearch) || (q.tag || '').toLowerCase().includes(lowerSearch);
    ...
    const quizTargetSubject = (currentSubjectList || []).find(s => s.id === selectedQuizSubjectId) || (currentSubjectList || [])[0];
    ...
    const handleGenerateQuiz = async () => {
        ...
        try {
            ...
            const newSubjects = currentSubjectList.map(s => {
                if (s.id !== selectedQuizSubjectId) return s;
                return {
                    ...s,
                    chapters: (s.chapters || []).map(c => {
                        // ⚡ 極速優化：只針對有抽到題目的章節進行重繪，節省 React 渲染與記憶體
            ...
            // ⚡ 極速優化：只在背景更新「有被抽到題目」的章節，且不等待伺服器回應
            if (activeMainTab === 'public') {
            ...
            const quizData = {
                ...
                isIndependentQuestions: true, // 標記為獨立物件架構
            ...
            // ⚡ 極速優化：改為背景非同步上傳，不等待伺服器回應，直接進入作答畫面
        ...
        <div className="flex h-full w-full bg-[#FCFBF7] dark:bg-stone-900 transition-colors">
            <div className="w-64 border-r border-stone-200 dark:border-stone-700 flex flex-col bg-stone-50 dark:bg-stone-900 shrink-0">
                <div className="flex bg-stone-100 dark:bg-stone-800 border-b border-stone-200 dark:border-stone-700">
                    <button onClick={() => setActiveMainTab('my')} className={`flex-1 py-2 font-black text-[13px] flex flex-col items-center justify-center gap-1 transition-colors ${activeMainTab === 'my' ? 'bg-white dark:bg-stone-900 text-stone-800 dark:text-stone-100 border-b-2 border-amber-500' : 'text-stone-500 hover:bg-stone-200 dark:hover:bg-stone-700'}`}><span className="material-symbols-outlined text-[18px]">library_books</span> 我的題庫</button>
                    <button onClick={() => setActiveMainTab('public')} className={`flex-1 py-2 font-black text-[13px] flex flex-col items-center justify-center gap-1 transition-colors ${activeMainTab === 'public' ? 'bg-white dark:bg-stone-900 text-stone-800 dark:text-stone-100 border-b-2 border-amber-500' : 'text-stone-500 hover:bg-stone-200 dark:hover:bg-stone-700'}`}><span className="material-symbols-outlined text-[18px]">public</span> 公開題庫</button>
                    <button onClick={() => setActiveMainTab('prompt')} className={`flex-1 py-2 font-black text-[13px] flex flex-col items-center justify-center gap-1 transition-colors ${activeMainTab === 'prompt' ? 'bg-white dark:bg-stone-900 text-stone-800 dark:text-stone-100 border-b-2 border-amber-500' : 'text-stone-500 hover:bg-stone-200 dark:hover:bg-stone-700'}`}><span className="material-symbols-outlined text-[18px]">smart_toy</span> Prompt</button>
                </div>

                {activeMainTab === 'my' || activeMainTab === 'public' ? (
                    <>
                        <div className="p-3 flex justify-between items-center bg-white dark:bg-stone-800 border-b border-stone-200 dark:border-stone-700">
                            <span className="font-bold text-sm text-stone-600 dark:text-stone-300">{activeMainTab === 'public' ? '公開題庫 (免費)' : '科目列表'}</span>
                        ...
                        <div className="flex-1 overflow-y-auto custom-scrollbar p-2 space-y-2">
                            {(currentSubjectList || []).length === 0 && <div className="text-sm text-gray-400 text-center py-4">目前尚無科目</div>}
                                                    ...
                                                    {(activeMainTab === 'my' || user?.email === 'jay03wn@gmail.com') && (
                                                        <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0" style={{ opacity: activeChapterId === chap.id ? 1 : undefined }}>
                                                           <button onClick={(e) => { e.stopPropagation(); handleMoveChapter(subj.id, chapIdx, 'up'); }} className="text-gray-400 hover:text-cyan-600" title="上移章節"><span className="material-symbols-outlined text-[14px]">arrow_upward</span></button>
                                                            ...
                                                            {user?.email === 'jay03wn@gmail.com' && activeMainTab === 'my' && (
                        ...
                        <div className="flex-1 overflow-y-auto custom-scrollbar p-6 flex flex-col">
                            <div className="flex justify-between items-end mb-4 border-b border-stone-200 dark:border-stone-700 pb-4 shrink-0">
                                ...
                                <div className="flex items-center gap-2">
                                    {(activeChapter.questions || []).length > 0 && (
                                        <button onClick={() => { setSelectedQuizSubjectId(activeSubjectId); setSelectedQuizChapterIds([activeChapterId]); setQuizNameInput(''); setShowQuizModal(true); }} className="bg-cyan-600 hover:bg-cyan-700 text-white font-bold text-sm px-4 py-1.5 rounded-lg shadow-sm flex items-center gap-1 transition-transform active:scale-95">
                            ...
                            <div className="flex flex-wrap sm:flex-nowrap gap-2 mb-6 bg-white dark:bg-stone-800 p-3 rounded-xl border border-stone-200 dark:border-stone-700 shadow-sm shrink-0">
                                <div className="flex-1 relative flex items-center gap-2">
                                    <div className="relative flex-1">
                                        ...
                                        <input type="text" placeholder="搜尋題目、標籤或詳解..." value={searchQ} onChange={e => setSearchQ(e.target.value)} className="w-full pl-9 pr-3 py-1.5 bg-stone-50 dark:bg-stone-900 border border-stone-300 dark:border-stone-600 rounded-lg outline-none text-sm dark:text-white" />
                                    </div>
                                    <label className="flex items-center gap-1.5 cursor-pointer text-sm font-bold text-stone-600 dark:text-stone-300 shrink-0 bg-stone-100 dark:bg-stone-700 px-3 py-1.5 rounded-lg border border-stone-200 dark:border-stone-600 hover:bg-stone-200 transition-colors">
                                        <input type="checkbox" checked={isGlobalSearch} onChange={e => setIsGlobalSearch(e.target.checked)} className="w-4 h-4 accent-amber-500" />
                                        跨庫搜尋
                                    </label>
                                </div>
                            ...
                            {(activeMainTab === 'my' || user?.email === 'jay03wn@gmail.com') && (
                                <div className="mb-6 bg-white dark:bg-stone-800 rounded-xl border border-stone-200 dark:border-stone-700 p-4 shadow-sm shrink-0">
                                    ...
                                    <textarea value={importText} onChange={(e) => setImportText(e.target.value)} placeholder="請貼上符合格式的題目...&#10;範例：&#10;[Q.001]&#10;[#標籤|@難度|Ans:A]&#10;題目文字...&#10;[A]選項A&#10;[B]選項B&#10;[C]選項C&#10;[D]選項D&#10;[Explain:詳解]" className="w-full h-24 p-3 bg-stone-50 dark:bg-stone-900 border border-stone-300 dark:border-stone-600 rounded-lg outline-none focus:border-amber-500 font-mono text-sm resize-none custom-scrollbar dark:text-stone-200" />
                            ...
                            <div className="space-y-4">
                               {displayedQuestions.length === 0 ? (
                                    ...
                                    <div key={q.id} className="bg-white dark:bg-stone-800 rounded-xl border border-stone-200 dark:border-stone-700 p-4 shadow-sm relative group">
                                        <div className="font-black text-stone-400 text-xs mb-2">題號: {isGlobalSearch ? `[${q._subjName} > ${q._chapName}] ` : ''}{idx + 1}</div>
                                        {(activeMainTab === 'my' || user?.email === 'jay03wn@gmail.com') && (
                                        ...
                                        <div className="flex flex-wrap gap-2 mb-3 pr-20">
                                            ...
                                            {isGlobalSearch && <span className="text-xs font-bold bg-purple-100 text-purple-800 dark:bg-purple-900/40 dark:text-purple-300 px-2 py-0.5 rounded-full border border-purple-200 dark:border-purple-800 flex items-center gap-1 shadow-sm"><span className="material-symbols-outlined text-[12px]">folder_open</span>{q._subjName} - {q._chapName}</span>}
                ...
                <div className="fixed inset-0 z-[150] bg-stone-900/60 backdrop-blur-sm flex items-center justify-center p-4">
                    <div className="bg-[#FCFBF7] dark:bg-stone-800 w-full max-w-sm rounded-2xl shadow-2xl border border-stone-200 dark:border-stone-700 p-6 flex flex-col">
                        ...
                        <p className="text-sm font-bold text-gray-600 dark:text-gray-300 mb-6">
                            您即將把「<span className="text-amber-600">{isPublishing.chap.name}</span>」推送到公開題庫。<br/><br/>
            ...
            {showQuizModal && (
                <div className="fixed inset-0 z-[150] bg-stone-900/60 backdrop-blur-sm flex items-center justify-center p-4">
                    <div className="bg-[#FCFBF7] dark:bg-stone-800 w-full max-w-lg max-h-[90vh] overflow-y-auto custom-scrollbar rounded-2xl shadow-2xl border border-stone-200 dark:border-stone-700 p-6 flex flex-col">
                        ...
                        <div className="mb-4">
                            <label className="block text-sm font-bold text-stone-700 dark:text-stone-300 mb-1.5">1. 選擇出題科目</label>
                            <select value={selectedQuizSubjectId || ''} onChange={e => { setSelectedQuizSubjectId(e.target.value); const subj = currentSubjectList.find(s => s.id === e.target.value); setSelectedQuizChapterIds(subj ? subj.chapters.map(c => c.id) : []); }} className="w-full p-2.5 bg-white dark:bg-stone-900 border border-stone-300 dark:border-stone-600 rounded-lg outline-none font-bold dark:text-white cursor-pointer focus:border-cyan-500">
                                {currentSubjectList.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
                            </select>
                        </div>

                        {quizTargetSubject && (
                            <div className="mb-6">
                                <label className="block text-sm font-bold text-stone-700 dark:text-stone-300 mb-1.5">2. 選擇出題章節 (可多選)</label>
