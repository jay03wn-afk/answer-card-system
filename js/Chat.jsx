...
function SocialDashboard({ user, userProfile, showAlert, showPrompt }) {
    ...
    // ✨ 新增：同時刪除 Storage 裡的實體圖片檔案，避免佔用雲端空間
    if (data.storagePath) {
        window.storage.ref(data.storagePath).delete().catch(e => console.log("Storage 圖片已清理或不存在"));
    }
    doc.ref.delete().catch(e => console.error("刪除文件失敗", e));
} else {
    ...
    // 5. 開放圖片上傳並限制 5 張過期機制
    // 5. 開放圖片上傳並限制 5 張過期機制 (已修正為上傳至 Firebase Storage)
const handleImageUpload = (e) => {
    if (!activeChat) return;
    const file = e.target.files[0];
    if (!file) return;

    window.showToast("圖片處理中...", "loading"); // 加上提示讓體驗更好
    ...
    reader.onload = (event) => {
        ...
        img.onload = async () => {
            ...
            try {
                // ✨ 核心修正：1. 將壓縮後的 Base64 上傳到 Firebase Storage
                // 路徑設計為 chat_images/聊天室ID/時間戳.jpg
                const imageRef = window.storage.ref(`chat_images/${chatId}/${Date.now()}.jpg`);
                await imageRef.putString(compressedBase64, 'data_url');
                ...
                if (imgSnap.size >= 5) {
                     const oldDocs = imgSnap.docs.slice(4); 
                     const batch = window.db.batch();
                     oldDocs.forEach(d => batch.update(d.ref, { expired: true, text: '(圖片已過期)' }));
                     await batch.commit();
                }

                // ✨ 核心修正：3. 存入 Firestore (改用 URL，而不是超長字串)
                await chatRef.add({
                    type: 'image',
                    imageUrl: downloadUrl, // 替換為真實的 Storage 網址
                    storagePath: imageRef.fullPath, // 紀錄路徑，方便過期時連同檔案一起刪除
                    senderId: user.uid,
                    senderName: userProfile.displayName,
                    timestamp: firebase.firestore.FieldValue.serverTimestamp(),
                ...
                window.showToast("圖片上傳成功！", "success");
            } catch (err) {
                window.showToast("圖片上傳失敗", "error");
                                        ...
                                        <div className={`max-w-[85%] md:max-w-[70%] p-3 text-sm rounded-2xl ${isMe ? 'bg-stone-800 text-white dark:bg-amber-600' : 'bg-[#FCFBF7] border border-stone-200 dark:border-stone-700 text-stone-800 dark:bg-gray-700 dark:text-white shadow-sm'}`}>
                                            {(() => {
                                                if (msg.type === 'image') {
                                                    ...
                                                    const qaMatch = msg.text?.match(/\?qaId=([a-zA-Z0-9_-]+)/);
                                                    const newsMatch = msg.text?.match(/\?newsId=([a-zA-Z0-9_-]+)/);
                                                    if (qaMatch) {
                                                        const cleanText = msg.text.replace(/https?:\/\/[^\s]+qaId=[a-zA-Z0-9_-]+/g, '').trim();
                                                        return (
                                                            <div className="flex flex-col gap-2">
                                                                <p className="break-all sm:break-words whitespace-pre-wrap">{cleanText || '分享了快問快答'}</p>
                                                                <button onClick={() => window.dispatchEvent(new CustomEvent('openFastQA', { detail: qaMatch[1] }))} className="bg-amber-500 text-white font-bold px-3 py-1.5 rounded-xl text-xs flex items-center justify-center gap-1 hover:bg-amber-600 transition-colors shadow-sm">
                                                                    <span className="material-symbols-outlined text-[16px]">bolt</span> 立即挑戰快問快答
                                                                </button>
                                                            </div>
                                                        );
                                                    } else if (newsMatch) {
                                                        const cleanText = msg.text.replace(/https?:\/\/[^\s]+newsId=[a-zA-Z0-9_-]+/g, '').trim();
                                                        return (
                                                            <div className="flex flex-col gap-2">
                                                                <p className="break-all sm:break-words whitespace-pre-wrap">{cleanText || '分享了電子報'}</p>
                                                                <button onClick={() => window.dispatchEvent(new CustomEvent('openNews', { detail: newsMatch[1] }))} className="bg-amber-500 text-white font-bold px-3 py-1.5 rounded-xl text-xs flex items-center justify-center gap-1 hover:bg-amber-600 transition-colors shadow-sm">
                                                                    <span className="material-symbols-outlined text-[16px]">newspaper</span> 立即閱讀電子報
                                                                </button>
                                                            </div>
                                                        );
                                                    }
