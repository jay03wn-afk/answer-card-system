...
if (docs.length > 50) {
    ...
    const handleClaimReward = async (e, msg) => {
        ...
        try {
            ...
            // ✨ 樂觀更新：立刻更新本地前端狀態，右上角鑽石與按鈕會瞬間改變，無需等待資料庫回傳
            setUserProfile(prev => ({ 
                ...prev, 
                mcData: { ...(prev?.mcData || {}), diamonds: (prev?.mcData?.diamonds || 0) + msg.rewardDiamonds } 
            }));
        ...
        {activeMailTab === 'important' && <div className="absolute bottom-0 left-0 w-full h-1 bg-rose-500 rounded-t-lg"></div>}
            ...
            {activeTab !== 'activeQuiz' ? (
                <div className="flex-grow pt-4 md:pt-6 overflow-hidden flex flex-col bg-gray-50 dark:bg-stone-900 transition-colors">
                    ...
                    {/* 加入這行 */}
                    {activeTab === 'druglib' && <DrugLibrary user={user} userProfile={userProfile} showAlert={showAlert} showConfirm={showConfirm} showPrompt={showPrompt} />}
