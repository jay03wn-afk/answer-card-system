        ...
        <div className="fixed inset-0 bg-black bg-opacity-80 flex items-center justify-center z-[70] p-4">
            <div className="bg-white dark:bg-gray-800 p-8 w-full max-w-lg no-round shadow-2xl relative border-t-4 border-black dark:border-gray-500 animate-fade-in-up">
                ...
                <div className="space-y-6 text-gray-700 dark:text-gray-300 mb-8">
                    <p className="text-sm">歡迎使用！雙螢幕排版、社群交流與 Minecraft 養成系統已準備就緒。</p>
                </div>
                <button onClick={onComplete} className="w-full bg-black dark:bg-gray-200 text-white dark:text-black p-3 font-bold no-round hover:bg-gray-800 dark:hover:bg-gray-300 text-lg transition-colors">🚀 開始體驗</button>
            </div>
        </div>
    );
}
...
function AuthScreen({ showAlert }) {
    ...
    const handleAuth = (e) => {
        ...
        const request = isSignUp ? auth.createUserWithEmailAndPassword(safeEmail, password) : auth.signInWithEmailAndPassword(safeEmail, password);
        
        request.then((cred) => {
            if (isSignUp) {
                const initMcData = { diamonds: 0, level: 1, exp: 0, hunger: 10, items: [], cats: 0, lastCheckIn: null };
                return db.collection('users').doc(cred.user.uid).set({ 
                    email: safeEmail, friends: [], unreadChats: {}, folders: ['未分類'], mcData: initMcData, hasSeenTutorial: false, avatar: null, bio: ""
                }).catch(err => showAlert("註冊成功，但建立資料庫檔案失敗: " + err.message));
            } else {
                db.collection('users').doc(cred.user.uid).set({ email: safeEmail }, { merge: true });
            }
        }).catch(err => showAlert('認證失敗：' + err.message)).finally(() => setLoading(false));
    };

    return (
        <div className="flex h-full items-center justify-center p-4 bg-gray-100 dark:bg-gray-900 overflow-y-auto">
            <form onSubmit={handleAuth} className="bg-white dark:bg-gray-800 p-8 shadow-md w-full max-w-sm no-round border border-gray-200 dark:border-gray-700">
                ...
                <input type="email" placeholder="電子郵件" className="w-full mb-3 p-3 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-black dark:text-white no-round" value={email} onChange={e => setEmail(e.target.value)} required />
                <input type="password" placeholder="密碼" className="w-full mb-6 p-3 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-black dark:text-white no-round" value={password} onChange={e => setPassword(e.target.value)} required />
                <button type="submit" className="w-full bg-black dark:bg-gray-200 text-white dark:text-black p-3 font-bold no-round" disabled={loading}>{loading ? '處理中...' : (isSignUp ? '註冊' : '登入')}</button>
...
function ProfileSetup({ user, onComplete, showAlert }) {
    ...
    return (
        <div className="flex h-full items-center justify-center p-4 bg-gray-50 dark:bg-gray-900">
            <div className="bg-white dark:bg-gray-800 p-8 shadow-md w-full max-w-sm no-round text-center">
                <h2 className="text-xl font-bold mb-4 dark:text-white">設定社群暱稱</h2>
                <input type="text" placeholder="例如：藥神" className="w-full mb-6 p-3 border bg-white dark:bg-gray-700 text-black dark:text-white no-round text-center" value={name} onChange={e => setName(e.target.value)} />
...
function ProfilePage({ user, userProfile, showAlert }) {
    ...
    const handleImageUpload = (e) => {
        ...
        reader.onload = (event) => {
            ...
            img.onload = () => {
                ...
                db.collection('users').doc(user.uid).update({ avatar: compressedBase64 })
    ...
    const saveBio = () => {
        setIsSaving(true);
        db.collection('users').doc(user.uid).update({ bio })
    ...
    return (
        <div className="max-w-3xl mx-auto p-6 mt-10 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-sm">
            <h2 className="text-2xl font-black mb-6 dark:text-white">個人檔案設定</h2>
            <div className="flex flex-col md:flex-row items-center md:items-start gap-8">
                <div className="flex flex-col items-center">
                    ...
                    <button onClick={() => fileInputRef.current.click()} className="text-sm font-bold bg-gray-100 dark:bg-gray-700 px-4 py-2 hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors dark:text-white">更換大頭照</button>
                    <input type="file" accept="image/*" className="hidden" ref={fileInputRef} onChange={handleImageUpload} />
                </div>
                <div className="flex-grow w-full">
                    <label className="block text-sm font-bold text-gray-500 mb-2">社群暱稱</label>
                    <div className="w-full mb-4 p-3 border bg-gray-50 dark:bg-gray-900 text-gray-500 dark:text-gray-400 cursor-not-allowed">{userProfile.displayName}</div>
                    
                    <label className="block text-sm font-bold text-gray-500 mb-2">關於我 (自我介紹)</label>
                    <textarea value={bio} onChange={e => setBio(e.target.value)} className="w-full h-32 mb-4 p-3 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-black dark:text-white outline-none resize-none" placeholder="寫點關於你自己的事吧..."></textarea>
                    
                    <button onClick={saveBio} disabled={isSaving} className="bg-black dark:bg-gray-200 text-white dark:text-black px-6 py-2 font-bold hover:bg-gray-800 transition-colors">
                        {isSaving ? '儲存中...' : '儲存自介'}
                    </button>
                </div>
            </div>
