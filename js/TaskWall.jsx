...
function TaskWallDashboard({ user, showAlert, showConfirm, onContinueQuiz }) {
    ...
    const renderTaskCard = (task, isOp = false) => {
        ...
        const borderClass = isOp ? 'border-yellow-300 dark:border-yellow-700' : 'border-indigo-200 dark:border-indigo-800';

        return (
            <div key={task.id} className={`border ${borderClass} p-4 bg-white dark:bg-gray-800 flex flex-col justify-between gap-3 hover:shadow-md transition-all no-round relative`}>
                <div className="flex flex-col gap-1 min-w-0 flex-grow">
                    ...
                    <div className="flex flex-wrap items-center gap-3 text-xs shrink-0 mt-2">
                        <span className="text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded">{task.numQuestions} 題</span>
                        ...
                        {isCompleted ? (
                            <span className="text-green-600 dark:text-green-400 font-bold ml-auto text-sm">✅ {localRec.results.score} 分</span>
                        ) : inProgress ? (
                            <span className="text-orange-500 dark:text-orange-400 font-bold ml-auto">📝 已填 {localRec.userAnswers.filter(a => a).length}</span>
                        ) : (
                ...
                <button 
                    ...
                    className={`py-2 px-4 no-round font-bold text-sm transition-colors mt-2 w-full text-center ${isCompleted ? 'bg-green-100 text-green-800 border border-green-300 hover:bg-green-200' : 'bg-black dark:bg-gray-200 text-white dark:text-black hover:bg-gray-800'}`}
                >
           ...
           {loading && Object.keys(opTasks).length === 0 && Object.keys(mnTasks).length === 0 ? (
                ...
                <div className="space-y-8 pb-10">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8 items-start">
                        ...
                        {/* 左側：國考題區塊 */}
                        <div className="bg-gradient-to-br from-yellow-50 to-white dark:from-gray-800 dark:to-gray-900 border border-yellow-400 dark:border-yellow-600 shadow-md no-round p-4 sm:p-6 w-full relative min-h-[300px]">
                            <h2 className="text-2xl font-black mb-6 dark:text-white border-b-2 border-yellow-400 dark:border-yellow-600 pb-2 text-yellow-700 dark:text-yellow-400 flex items-center">
                                🏆 歷屆國考題
                            </h2>
                            ...
                            {isSearching ? (
                                ...
                                <div className="grid grid-cols-3 sm:grid-cols-4 gap-2 sm:gap-3 animate-fade-in">
                                    {sortedYears.map(year => (
                                        <button 
                                            ...
                                            className="bg-white dark:bg-gray-700 border-2 border-yellow-300 dark:border-yellow-600 py-3 sm:py-4 px-1 text-center font-bold text-yellow-800 dark:text-yellow-400 hover:bg-yellow-100 dark:hover:bg-gray-600 transition-colors shadow-sm active:scale-95"
                                        >
                                            {year}
                                        </button>
                                    ))}
                                ...
                                <div className="animate-fade-in">
                                    <div className="flex justify-between items-center mb-4 bg-yellow-100 dark:bg-yellow-900/30 p-2 border-l-4 border-yellow-500">
                                        <h3 className="text-lg font-black text-yellow-800 dark:text-yellow-300">{selectedYear}</h3>
                                        <button onClick={() => setSelectedYear(null)} className="text-sm bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 px-3 py-1 font-bold shadow-sm hover:bg-gray-100 transition-colors">
                                            🔙 返回年份
                                        </button>
                                    </div>
                                    <div className="space-y-4">
                        ...
                        {/* 右側：模擬試題區塊 */}
                        <div className="bg-white dark:bg-gray-800 border border-indigo-300 dark:border-indigo-800 shadow-md no-round p-4 sm:p-6 w-full relative min-h-[300px]">
                            ...
                            {isSearching ? (
                                ...
                                <div className="grid grid-cols-2 gap-3 animate-fade-in">
                                    {sortedSubjects.map(subj => (
                                        ...
                                        >
                                            ...
                                            <span className="text-xs bg-white dark:bg-gray-800 px-1.5 rounded-full">{Object.values(mnTasks[subj]).flat().length}</span>
                                        </button>
                                    ))}
                                ...
                                <div className="animate-fade-in">
                                    <div className="flex justify-between items-center mb-6 bg-indigo-50 dark:bg-indigo-900/30 p-2 border-l-4 border-indigo-500">
                                        ...
                                        <button onClick={() => setSelectedSubject(null)} className="text-sm bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 px-3 py-1 font-bold shadow-sm hover:bg-gray-100 transition-colors shrink-0">
                                            🔙 返回科目
                                        </button>
                                    </div>
                                    <div className="space-y-8">
                                        {Object.keys(mnTasks[selectedSubject]).map(tag => (
                                            <div key={tag} className="pl-3 border-l-2 border-indigo-200 dark:border-indigo-700">
                                                <h4 className="text-md font-bold mb-3 text-gray-600 dark:text-gray-300 inline-block bg-gray-100 dark:bg-gray-700 px-3 py-1">
                                                    🏷️ {tag}
                                                </h4>
                    ...
                    <div className="flex justify-center mt-8 pt-4">
                        <button 
                            ...
                            className="bg-white dark:bg-gray-800 border-2 border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 px-8 py-3 font-bold shadow-md hover:bg-gray-50 dark:hover:bg-gray-700 transition-all active:scale-95"
                        >
                            ⬇️ 載入更早的任務...
                        </button>
                    </div>
